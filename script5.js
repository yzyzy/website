...
function startGame() {
	...
	origBoard = Array.from(Array(25).keys()) /*create an array from 0 to 9*/
...
function turnClick(square) { /* pass in a click event to the turnClick function*/
	...
	if (typeof origBoard[square.target.id] == 'number') {/* cannot click on a place that's already been clicked. 
		...
		turn1(square.target.id, huPlayer) /* call and pass id to the turn function*/
		if (!checkTie()) turn2(bestSpot(), aiPlayer); /* check if the game is tie, if it is, AI doesn't take turn; otherwise it does*/
	}
}
...
function turn1(squareId, player) {
	...
	// origBoard[squareId+1] = player; /* set origBard array value to player*/
	// document.getElementById(squareId-1).innerText = Number(squareId)-1; /* squareId are 0 to 9, set by html*/

	if (typeof origBoard[Number(squareId)-1] != 'number' && ![0,5,10,15,20].includes(Number(squareId))){
		if (origBoard[Number(squareId)-1] == huPlayer) {
			origBoard[Number(squareId)-1] = aiPlayer
			document.getElementById(Number(squareId)-1).innerText = aiPlayer;
		}
		else if (origBoard[Number(squareId)-1] == aiPlayer) {
			origBoard[Number(squareId)-1] = huPlayer
			document.getElementById(Number(squareId)-1).innerText = huPlayer;
		}
	}

	if (typeof origBoard[Number(squareId)+1] != 'number' && ![4,9,14,19,24].includes(Number(squareId))){
		if (origBoard[Number(squareId)+1] == huPlayer) {
			origBoard[Number(squareId)+1] = aiPlayer
			document.getElementById(Number(squareId)+1).innerText = aiPlayer;
		}
	...
	if (typeof origBoard[Number(squareId)-5] != 'number'){
		if (origBoard[Number(squareId)-5] == huPlayer) {
			origBoard[Number(squareId)-5] = aiPlayer
			document.getElementById(Number(squareId)-5).innerText = aiPlayer;
		}
		else if (origBoard[Number(squareId)-5] == aiPlayer) {
			origBoard[Number(squareId)-5] = huPlayer
			document.getElementById(Number(squareId)-5).innerText = huPlayer;
		}
	}
...
function bestSpot(){
	...
	/* using minimax for 5by5 is too computationally intensive, do random number instead*/
